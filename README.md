...
Если переменные не заданы, приложение использует `config/config.json` для режима `development`.
...
- Если вы видите AccessDeniedError — проверьте `DB_USER` и `DB_PASSWORD` в `.env` или `config/config.json`.
